...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                               ...
                               QLineEdit, QFileDialog, QSystemTrayIcon, QStyle, QMessageBox, QSizePolicy, QSpacerItem)
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.notification_signal.emit("Error", "Failed to open Excel file. Please make sure it is closed and try again.")
                self.finished_signal.emit(False)
                try: excel.Quit()
                except: pass
                return
                    ...
                    if stage == "waiting_images_link":
                        ...
                        if result.get("image_src"):
                            ...
                            try:
                                ...
                                self.status_signal.emit(f"Warning: Failed to save Excel file - {e}")
                            
                            entry["stage"] = "delaying_next_row"
                            entry["next_action_time"] = current_time + 2.0
                            try: page.evaluate("document.body.innerHTML = '<h2>Image Found! Waiting 2s...</h2>';")
                            except: pass
                            continue
